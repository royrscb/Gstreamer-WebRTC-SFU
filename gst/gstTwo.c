 ...
 * Author: Roy Ros Cobo
 *
 * $ gcc gstTwo.c $(pkg-config --cflags --libs gstreamer-webrtc-1.0 gstreamer-sdp-1.0 libsoup-2.4 json-glib-1.0) -o gstTwo
 *
 * This application resends the media between the peers directly.
*/
...
static void link_pattern_to_newWrbin(gint newWrbin_ownerPeer, gint newWrbin_ownerPipe){

    GstElement *outBin;
    if(peers[newWrbin_ownerPipe].id == 1) outBin = gst_parse_bin_from_description(
      "videotestsrc ! queue ! "VIDEO_COD" ! webrtcbin name=newBin"
      , TRUE, NULL);
    else if(peers[newWrbin_ownerPipe].id == 2) outBin = gst_parse_bin_from_description(
      "videotestsrc pattern=ball ! queue ! "VIDEO_COD" ! webrtcbin name=newBin"
      , TRUE, NULL);
    else if(peers[newWrbin_ownerPipe].id == 3) outBin = gst_parse_bin_from_description(
      "videotestsrc pattern=snow ! queue ! "VIDEO_COD" ! webrtcbin name=newBin"
      , TRUE, NULL);

    gst_bin_add(GST_BIN (peers[newWrbin_ownerPipe].pipel), outBin); //add to proper pipe

    // Getting the webrtcbin to store it and set signal pads
    gint newWrbinIndex = peers[newWrbin_ownerPeer].nwrbins; 

    peers[newWrbin_ownerPeer].wrbins[newWrbinIndex].wrbin = gst_bin_get_by_name(GST_BIN (outBin), "newBin");
    peers[newWrbin_ownerPeer].wrbins[newWrbinIndex].ownerPeer = newWrbin_ownerPeer;
    peers[newWrbin_ownerPeer].wrbins[newWrbinIndex].ownerPipe = newWrbin_ownerPipe; 
    
    peers[newWrbin_ownerPeer].nwrbins++;


    // Connect signals and goes to play
    userData *usDa = getConstUsDa(newWrbin_ownerPeer, newWrbinIndex);
...
//This creates a new wrbin from newWrbin_ownerPeer into the newWrbin_ownerPipe pipeline and link the entry stream as the new pad.
static void link_newSrcPad_to_newWrbin(gint newWrbin_ownerPeer, gint newWrbin_ownerPipe){

    GstPad *new_srcPad = peers[newWrbin_ownerPipe].wrbins[0].srcPad;
    ...
    GstStateChangeReturn ret = gst_pad_link (new_srcPad, sinkPad);


    // Debug
    const gchar *sinkPad_type = gst_structure_get_name(gst_caps_get_structure (gst_pad_query_caps (sinkPad, NULL), 0));
    const gchar *new_srcPad_type = gst_structure_get_name(gst_caps_get_structure (gst_pad_query_caps (new_srcPad, NULL), 0));

    if(GST_PAD_LINK_FAILED (ret)) g_print("\n\n\n\nLINK FAILED!\n\n");
    else g_print("\n\n\nLINK SUCCESFULLY!\n\n");

    if(gst_pad_is_linked(new_srcPad)) g_print("new_pad type:%s YES linked!\n", new_srcPad_type); 
    ...
    peers[newWrbin_ownerPeer].wrbins[newWrbinIndex].wrbin = gst_bin_get_by_name(GST_BIN (outBin), "newBin");
    peers[newWrbin_ownerPeer].wrbins[newWrbinIndex].sinkPad = sinkPad;
    peers[newWrbin_ownerPeer].wrbins[newWrbinIndex].ownerPeer = newWrbin_ownerPeer;
    peers[newWrbin_ownerPeer].wrbins[newWrbinIndex].ownerPipe = newWrbin_ownerPipe; 
...
static void on_pad_added(GstElement *webrtc, GstPad *new_pad, userData *usDa){
  ...
  if (usDa->index != 0 || GST_PAD_DIRECTION (new_pad) != GST_PAD_SRC) return;
  else  peers[usDa->peerID].wrbins[0].srcPad = new_pad;
...
static void on_sign_message(SoupWebsocketConnection *ws_conn, SoupWebsocketDataType dataType, GBytes *message, gpointer user_data){
  ...
  if(g_strcmp0(type,"txt")==0) g_print("%s\n", json_object_get_string_member(data,"data"));
    ...
    const char *ip = json_object_get_string_member(data_data, "ip");
